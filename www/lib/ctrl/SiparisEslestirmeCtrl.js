function SiparisEslestirmeCtrl($scope,$window,$timeout,db)
{
    ...
    function Init()
    {
        ...
        $scope.StokListeSelectedIndex = -1;
    ...
    function BarkodGetir(pBarkod)
    {
        if(pBarkod != '')
        {
            ...
            EslestirmeStokGetir(pBarkod,async function(pData,pTip)
            {
                ...
                {
                    MiktarLock = true
                    $timeout( function(){$scope.Insert()},200);  
                }
                else
                {
    ...
    {   
        ...
        console.log($scope.TopIskonto,(db.SumColumn($scope.StokHarListe,"sth_tutar")),db.SumColumn($scope.StokHarListe,"sth_vergi"),$scope.Stok[0].TUTAR)
        var CariHarUpdate = 
        [
            (((db.SumColumn($scope.StokHarListe,"sth_tutar") - $scope.TopIskonto) + db.SumColumn($scope.StokHarListe,"sth_vergi")) + $scope.Stok[0].TUTAR + $scope.Stok[0].KDV), //MEBLAG
    ...
    function StokHarInsert()
    {
        ...
        [
            ...
            0,  // VERGİSİZFL
            0,  // ADRESNO
            $scope.Stok[0].PARTI,  // PARTI
            $scope.Stok[0].LOT,  // LOT
            $scope.Proje,
            '', // EXİMKODU
            0,  // DİSTİCARETTURU
            0,  // OTVVERGİSİZFL
    ...
    $scope.EvrakTipChange = async function()
    {
        ...
        db.FillCmbDocInfo($scope.Firma,'CmbPersonelGetir',function(data)
        {
            ...
            $scope.Personel = EvrakParam.Personel;
    ...
    $scope.BtnCariListele = function()
    {
        ...
        // {
        //     let SKodu = '';
        //     if($scope.CmbStokAra == "1")
        //     {
        //         SKodu = $scope.TxtStokAra.replace("*","%").replace("*","%");
        //     }
        //     db.StokBarkodGetir($scope.Firma,SKodu,$scope.DepoNo,async function(BarkodData)
        //     {
        //         if(BarkodData.length > 0)
        //         {
        //             if(BarkodData[0].CARIKODU != "")
        //             {
        //                 $scope.CariStok = BarkodData;
        //                 console.log(BarkodData[0].CARIKODU)
        //                 db.GetData($scope.Firma,'CariListeGetir',[BarkodData[0].CARIKODU,'',UserParam.Sistem.PlasiyerKodu],function(data)
        //                 {
        //                     $scope.CariListe = data;   
    ...
    $scope.MiktarFiyatValid = function()
    {
        ...
        console.log($scope.Stok[0].INDIRIM,$scope.Stok[0].KDV,$scope.Stok[0].TOPTANVERGI,$scope.Stok[0].TOPTUTAR) 
    }
    ...
    $scope.BtnSiparisKabulListele = async function()
    {
        ...
        if($scope.SipSeriSira == "")
        {
            ...
            [
                ...
                UserParam.Sistem.SiparisOnayListele,
                $scope.CariKodu
            ];
    ...
    {
        let SeriSiraDizi = $scope.SipSeriSira.split("-")
        let TmpParam = [SeriSiraDizi[0],SeriSiraDizi[1],0];
    ...
    $scope.StokListeRowClick = function(pIndex,pItem,pObj)
    {
        ...
        $scope.StokListeSelectedIndex = pIndex;
    }
    ...
    $scope.BtnStokGridGetir = function()
    {
        ...
        console.log($scope.SipSira)
        db.GetData($scope.Firma,'SiparisListeGetir',[StokKodu,$scope.DepoNo,$scope.CariKodu,$scope.SipSeri,$scope.SipSira,0],function(StokData)
        {
