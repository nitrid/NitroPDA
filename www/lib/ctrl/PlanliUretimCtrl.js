...
{   
    ...
    {   
        db.GetData($scope.Firma,'PlanliUretimStokHarGetir',[$scope.Seri,$scope.Sira],function(data)
        {   
            if(data.length > 0)
            {
                Init();
    ...
    $scope.IsEmriListeRowClick = function(pIndex,pItem,pObj)
    {
        ...
        db.GetData($scope.Firma,'PlanliUretimListesiGetir',[$scope.IsEmriKodu],function(data)
            {
                if(data.length > 0)
                {
                    $scope.IsEmriPlanı = data
                    $("#TblIsEmri").jsGrid({data : $scope.IsEmriPlanı}); 
                }
                else
                {
    ...
    $scope.MainClick = function() 
    {
        ...
        $("#TbUretimOnay").removeClass('active');
    }
    ...
    $scope.UretimOnayClick = function()
    {
        if($scope.Sira == 0 || typeof $scope.Sira == "undefined")
        {            
            alertify.alert("<a style='color:#3e8ef7''>" + "Lütfen Evrak Siranın Gelmesini Bekleyin!" + "</a>" );
    ...
    $scope.BtnOnayla = async function()
    {
        if($scope.CmbEvrakTip == 0)
        {
            for(var i=0; i<$scope.IsEmriPlanı.length; i++)
            {
        ...
        alertify.alert("Stoklar başarıyla eklendi. Lütfen İşLem Satırlarından Kontrol Ediniz...") 
    }
