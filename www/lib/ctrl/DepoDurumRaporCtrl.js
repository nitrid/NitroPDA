...
{   
    ...
    $scope.BtnGetir = function()
    {
        var TmpQuery = 
        {
            ...
            "ISNULL((SELECT SUM(sth_miktar) FROM STOK_HAREKETLERI WHERE sth_stok_kod = STOKLAR.sth_stok_kod and sth_tip = 1 and sth_tarih >= @TARIH AND sth_cikis_depo_no = 4 ),0) AS SATILAN, " +
            "ISNULL((SELECT dbo.fn_DepodakiMiktar (STOKLAR.sth_stok_kod,4  ,CONVERT(VARCHAR(10),'" + $scope.SonTarih + "',112))),0) AS KALAN " +
    ...
    $scope.BtnFisYazdir = function()
    {
        ...
        {
            let FisDizayn = "";
            let i = 30 - $scope.FisLength.length;
            let Satır = "";
            ...
            $scope.FisData + "\n" //İÇERİK
            FisDizayn = FisDizayn + "   " +"YÜKLENEN: " + parseFloat(db.SumColumn($scope.FisLength,"SATILAN").toFixed(2)) + " " + "SATILAN: " + parseFloat(db.SumColumn($scope.FisLength,"SATILAN").toFixed(2)) + " " + "KALAN: " + parseFloat(db.SumColumn($scope.FisLength,"SATILAN").toFixed(2)) + "\n" +
            Satır 
            FisDizayn = FisDizayn +  "   "
            "-\n" + "-\n" + "-\n" + "-\n" + "-\n" + "-\n" + "-\n" + "-\n" + "-\n" + "-\n" + "-\n"
