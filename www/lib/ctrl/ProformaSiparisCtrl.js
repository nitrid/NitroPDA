function ProformaSiparisCtrl($scope,$window,$timeout,db,$filter)
{
    ...
    function DipToplamHesapla()
    {
        ...
        angular.forEach($scope.SiparisListe,function(value)
        {
            ...
            $scope.ToplamIndirim += (value.pro_iskonto1 + value.pro_iskonto2 + value.pro_iskonto3 + value.pro_iskonto4 + value.pro_iskonto5 + value.pro_iskonto6);
            $scope.ToplamKdv +=  (value.pro_tutari - (value.pro_iskonto1 + value.pro_iskonto2 + value.pro_iskonto3 + value.pro_iskonto4 + value.pro_iskonto5 + value.pro_iskonto6)) * (value.VERGIPNTR / 100);
        });
    ...
    {   
        ...
        db.ExecuteTag($scope.Firma,'ProformaSiparisInsert',InsertData,function(InsertResult)
        {          
    ...
    {   
        ...
        {
            db : '{M}.' + $scope.Firma,
            query:  "UPDATE PROFORMA_SIPARISLER SET sip_special1 = 1 " +
                    "WHERE pro_evrakno_seri = @pro_evrakno_seri AND pro_evrakno_sira = @pro_evrakno_sira AND pro_tip = @pro_tip ",
            param:  ['pro_evrakno_seri','pro_evrakno_sira','pro_tip'],
    ...
    $scope.YeniEvrak = async function (pAlinanVerilen)
    {
        //ALINAN = 0 VERİLEN = 1
        if(pAlinanVerilen == 0)
            ParamName = "ProformaAlinanSiparis";
        else
            ParamName = "ProformaVerilenSiparis";
        ...
        db.FillCmbDocInfo($scope.Firma,'CmbSorumlulukGetir',function(data)
       {
        ...
        await db.MaxSiraPromiseTag($scope.Firma,'MaxProformaSiparisSira',[$scope.Seri,$scope.EvrakTip,2],function(data)
        {
            $scope.Sira = data
        });

        if(UserParam.ProformaAlinanSiparis.SiparisOnay == "0")
        {
    ...
    {
        await db.MaxSiraPromiseTag($scope.Firma,'MaxProformaSiparisSira',[$scope.Seri,$scope.EvrakTip,2],function(data){$scope.Sira = data});
    }
    ...
    $scope.Insert = function()
    {
        ...
        {   
            if(UserParam.ProformaAlinanSiparis.EksiyeDusme == 1 &&  $scope.EvrakTip == 0 && ($scope.Miktar * $scope.Stok[0].CARPAN) > $scope.Stok[0].DEPOMIKTAR)
            {
    ...
    {
        db.GetData($scope.Firma,'SiparisProjeGetir',[$scope.EvrakTip,2,$scope.ProjeKod],function(data)
        {
    ...
    {
        db.GetData($scope.Firma,'ProformaSiparisGetir',[$scope.Seri,$scope.Sira,$scope.EvrakTip,2],function(data)
        {
            if(data.length > 0)
            {
                Init();
                ...
                $scope.Seri = data[0].pro_evrakno_seri;
                $scope.Sira = data[0].pro_evrakno_sira;
                $scope.EvrakTip = data[0].pro_tipi.toString();
                $scope.CariKodu = data[0].pro_mustkodu;
                $scope.BelgeNo = data[0].pro_belge_no;
                $scope.Tarih = new Date(data[0].pro_tarihi).toLocaleDateString();                
                $scope.Barkod = "";
                $scope.Stok = 
                [
                    {
                ...
                {    
                    $scope.DepoListe = e; 
                    $scope.DepoNo = data[0].pro_depono.toString();
                    $scope.DepoListe.forEach(function(item) 
                    {
                        if(item.KODU == $scope.DepoNo)
                            $scope.DepoAdi = item.ADI;
    ...
    $scope.EvrakDelete = function(pAlinanVerilen)
    {
        ...
        { 
            if($scope.SiparisListe.length > 0)
            {
                ...
                {
                    db.ExecuteTag($scope.Firma,'ProformaSiparisEvrDelete',[$scope.Seri,$scope.Sira,$scope.EvrakTip,2],function(data)
                    {
                        if(pAlinanVerilen == 0)
    ...
    $scope.SatirDelete = function(pAlinanVerilen)
    {
        ...
        { 
            if($scope.IslemListeSelectedIndex > -1)
            {
                if(UserParam[ParamName].EvrakSil == 0)
                {
                    db.ExecuteTag($scope.Firma,'ProformaSiparisSatirDelete',[$scope.SiparisListe[$scope.IslemListeSelectedIndex].pro_Guid],function(data)
                    {
